...
Public Class WImportarTablas

    Public lista As New List(Of SuperEntidad)
    Public aux As New TipoCambioRN
        ...
        If Me.dgvExcel.Rows.Count <> 1 Then
            For n As Integer = 0 To Me.dgvExcel.Rows.Count - 2
                ...
                If Me.dgvExcel.Item(0, n).Value.ToString <> "" Then
                    ...
                    entd.FechaTipoCambio = CType(Me.dgvExcel.Item(1, n).Value.ToString, Date)
                    entd.CompraTipoCambio = CType(Me.dgvExcel.Item(4, n).Value.ToString, Decimal)
                    entd.VentaTipoCambio = CType(Me.dgvExcel.Item(3, n).Value.ToString, Decimal)
                    entd.CompraEurTipoCambio = 1
                    entd.VentaEurTipoCambio = 1
                    entd.DolarEuroTipoCambio = 1
                    entd.MesTipoCambio = Me.dgvExcel.Item(2, n).Value.ToString
                    entd.AnoTipoCambio = wFecha.Year.ToString.PadLeft(4, CType("0", Char))
                    entd.EstadoRegistro = "1"
                    entd.Exporta = "0"
                    aux.nuevoTipoCambio(entd)

                End If

            Next
        End If
        MsgBox("La importacion TIPO CAMBIO se realizo con exito")
