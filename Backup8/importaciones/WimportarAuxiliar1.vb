...
Public Class WimportarAuxiliar1
    ...
    Sub Grabar()
        ...
        If Me.dgvExcel.Rows.Count <> 1 Then
            For n As Integer = 0 To Me.dgvExcel.Rows.Count - 2
                If Me.dgvExcel.Item(0, n).Value.ToString <> "" Then
                    ...
                    entd = aux.buscarAuxiliarExisPorCodigo(entd)
                    If entd.CodigoAuxiliar <> "" Then
                        entd.TipoAuxiliar = "1"
                        aux.modificarAuxiliar(entd)
                    Else
                        obj = New SuperEntidad
                        obj.CodigoAuxiliar = Me.dgvExcel.Item(0, n).Value.ToString
                        obj.DescripcionAuxiliar = Me.dgvExcel.Item(1, n).Value.ToString
                        lista.Add(obj)
                    End If
                End If
            Next
        ...
        'Dgv.refrescarFuenteDatosGrilla(Me.DataGridView1, lista)
        ''/Creando las columnas
        'Dgv.asignarColumnaText(Me.DataGridView1, cam.CodAux, "Codigo", 100)
        'Dgv.asignarColumnaText(Me.DataGridView1, cam.DesAux, "Razon Social / Nombre", 370)
        Dim m As Integer = -1
        For Each xobj As SuperEntidad In lista
            m += 1
            Me.DataGridView1.Rows.Add()
            Me.DataGridView1.Item(0, m).Value = xobj.CodigoAuxiliar
            Me.DataGridView1.Item(1, m).Value = xobj.DescripcionAuxiliar
        Next
