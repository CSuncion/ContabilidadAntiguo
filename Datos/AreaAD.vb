...
Public Class AreaAD
    ...
    Sub SetParametros(ByRef ent As SuperEntidad)
        '//
        Sql.AsignarParametro(Par.ClaveArea, ent.ClaveArea)
        Sql.AsignarParametro(Par.CodigoEmpresa, ent.CodigoEmpresa)
        Sql.AsignarParametro(Par.CodigoArea, ent.CodigoArea)
        Sql.AsignarParametro(Par.NombreArea, ent.NombreArea)
        'Flag File
        Select Case ent.EstadoArea
            Case "Activa"
                ent.EstadoArea = "1"
            Case "Cerrada"
                ent.EstadoArea = "0"
        End Select
        Sql.AsignarParametro(Par.EstadoArea, ent.EstadoArea)
    ...
    Function Objeto(ByRef iDr As IDataReader) As SuperEntidad
        ...
        objEnc.ClaveArea = iDr(Campo.ClaAre).ToString
        ...
        If iDr(Campo.EstAre).ToString = "1" Then
            objEnc.EstadoArea = "Activa"
        Else
            objEnc.EstadoArea = "Cerrada"
        End If
        ...
        'especial
        objEnc.CampoCursor = iDr(Campo.ClaAre).ToString
        Return objEnc
        '\\
    End Function
    ...
    Sub SpNuevoArea(ByRef ent As SuperEntidad)
        '//
        Try
            Sql.Conectar(SqlDatos.Bd.ContabilidadEmma)
            Sql.ComandoProcAlm("SpNuevoArea")
            '/Asignar Parametros
    ...
    Sub SpModificarArea(ByRef ent As SuperEntidad)
        '//
        Try
            Sql.Conectar(SqlDatos.Bd.ContabilidadEmma)
            Sql.ComandoProcAlm("SpModificarArea")
            '/Asignar Parametros
            Me.SetParametros(ent)
            '/Ejecutar sin resultado
            Sql.EjecutarSinResultado()
    ...
    Sub SpEliminarArea(ByRef ent As SuperEntidad)
        '//
        Try
            Sql.Conectar(SqlDatos.Bd.ContabilidadEmma)
            Sql.ComandoProcAlm("SpEliminarArea")
            '/Asignar Parametro
            Sql.AsignarParametro(Par.ClaveArea, ent.ClaveArea)
            '/Ejecutar sin resultado
            Sql.EjecutarSinResultado()
