...
Public Class FilesAD
    ...
    Sub SetParametros(ByRef ent As SuperEntidad)
        '//
        Sql.AsignarParametro(Par.ClaveFile, ent.ClaveFile)
        Sql.AsignarParametro(Par.CodigoEmpresa, ent.CodigoEmpresa)
        Sql.AsignarParametro(Par.CodigoFile, ent.CodigoFile)
        Sql.AsignarParametro(Par.NombreFile, ent.NombreFile)
        'Flag File
        Select Case ent.EstadoFile
            Case "Activa"
                ent.EstadoFile = "1"
            Case "Cerrada"
                ent.EstadoFile = "0"
        End Select
        Sql.AsignarParametro(Par.EstadoFile, ent.EstadoFile)
    ...
    Function Objeto(ByRef iDr As IDataReader) As SuperEntidad
        ...
        objEnc.ClaveFile = iDr(Campo.ClaFil).ToString
        ...
        objEnc.NombreFile = iDr(Campo.NomFil).ToString
        
        If iDr(Campo.EstFil).ToString = "1" Then
            objEnc.EstadoFile = "Activa"
        Else
            objEnc.EstadoFile = "Cerrada"
        End If
        ...
        'especial
        objEnc.CampoCursor = iDr(Campo.ClaFil).ToString
        Return objEnc
        '\\
    End Function
    ...
    Sub SpNuevoFiles(ByRef ent As SuperEntidad)
        '//
        Try
            Sql.Conectar(SqlDatos.Bd.ContabilidadEmma)
            Sql.ComandoProcAlm("SpNuevoFiles")
            '/Asignar Parametros
    ...
    Sub SpModificarFiles(ByRef ent As SuperEntidad)
        '//
        Try
            Sql.Conectar(SqlDatos.Bd.ContabilidadEmma)
            Sql.ComandoProcAlm("SpModificarFiles")
            '/Asignar Parametros
            Me.SetParametros(ent)
            '/Ejecutar sin resultado
            Sql.EjecutarSinResultado()
    ...
    Sub SpEliminarFiles(ByRef ent As SuperEntidad)
        '//
        Try
            Sql.Conectar(SqlDatos.Bd.ContabilidadEmma)
            Sql.ComandoProcAlm("SpEliminarFiles")
            '/Asignar Parametro
            Sql.AsignarParametro(Par.ClaveFile, ent.ClaveFile)
            '/Ejecutar sin resultado
            Sql.EjecutarSinResultado()
